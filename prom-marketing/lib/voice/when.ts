...
const NUMBER_WORDS: Record<string, number> = {
  ...
  // ⚠️ Дотук свършваше списъкът и „сряда в петнайсет" падаше в 10:00 по
  // подразбиране — тихо, без грешка. По телефона часовете след обяд се казват
  // и в двайсет и четири часовата форма („в осемнайсет"), особено от хора,
  // свикнали да пишат часове в документи.
  тринайсет: 13, тринадесет: 13, "13": 13,
  четиринайсет: 14, четиринадесет: 14, "14": 14,
  петнайсет: 15, петнадесет: 15, "15": 15,
  шестнайсет: 16, шестнадесет: 16, "16": 16,
  седемнайсет: 17, седемнадесет: 17, "17": 17,
  осемнайсет: 18, осемнадесет: 18, "18": 18,
  деветнайсет: 19, деветнадесет: 19, "19": 19,
  двайсет: 20, двадесет: 20, "20": 20,
  "21": 21,
  "22": 22,
  "23": 23,
};
...
/**
 * „в двайсет и едно" е един час, а не „двайсет" плюс нещо. Без това
 * съставните часове след двайсет се четат с един час назад.
 */
const COMPOUND_HOURS: Array<[RegExp, string]> = [
  [/два(?:й|де)сет\s+и\s+един(?:айсет)?\b/g, "21"],
  [/два(?:й|де)сет\s+и\s+едно/g, "21"],
  [/два(?:й|де)сет\s+и\s+два/g, "22"],
  [/два(?:й|де)сет\s+и\s+две/g, "22"],
  [/два(?:й|де)сет\s+и\s+три/g, "23"],
];

function normalizeHours(text: string): string {
  let out = text;
  for (const [re, val] of COMPOUND_HOURS) out = out.replace(re, val);
  return out;
}
...
export function parseWhen(raw: string, opts?: { now?: Date; defaultHour?: number }): ParsedWhen | null {
  ...
  const text = normalizeHours(raw.trim().toLowerCase());
  if (!text) return null;
