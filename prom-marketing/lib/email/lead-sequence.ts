...
export const LEAD_SEQUENCE: SequenceStep[] = [
  {
    key: "s1_lichno",
    afterDays: 0,
    subject: "Направих ти вход към демото — виж системата отвътре",
    build: (name, ctx) => {
      ...
      return {
        html: shell(`<p>${hi}</p>
<p>Радвам се, че остави телефона си на нашата реклама за AI автоматизации. Вместо да ти го обяснявам с думи, ето ти го наживо — отваря се от браузъра, нищо не се инсталира и нищо не се попълва:</p>
<p style="margin:22px 0"><a href="${demo}" style="display:inline-block;background:#0b6b4a;color:#fff;padding:14px 28px;border-radius:7px;text-decoration:none;font-weight:600;font-size:16px">Отвори демото →</a></p>
<p style="color:#4a5651;margin-top:-8px">Линкът е твой — по него виждам, че си влязъл, и следващия път говорим по конкретното, вместо да започваме отначало.</p>
<p>Вътре работят <strong>16 агента едновременно</strong>. Ето какво поемат:</p>
<ul style="padding-left:20px;margin:14px 0">
<li style="margin-bottom:10px"><strong>Запитвания и клиенти.</strong> Хващат лийда от рекламата, сайта или обаждането, оценяват го от 0 до 100, отговарят за секунди по имейл, чат и Messenger, и връщат замълчалите — за да не изтича нищо между пръстите.</li>
<li style="margin-bottom:10px"><strong>Документи и пари.</strong> Сглобяват офертата и я пращат като PDF, издават фактурата, следят кое е платено и кое още не е.</li>
<li style="margin-bottom:10px"><strong>Реклами и съдържание.</strong> Пишат постовете, генерират рекламните видеа от текст, публикуват в пет мрежи наведнъж, пълнят календара за седмицата и местят бюджета към кампанията, която печели.</li>
<li style="margin-bottom:10px"><strong>Ти, с глас.</strong> Казваш какво е станало по телефона — системата го записва, насрочва го и го изпълнява. Без да сядаш пред екран.</li>
</ul>
<p>Строим и неща извън демото: следене на камери и обекти, връзка с машини, автономни задачи, които вървят сами през нощта.</p>
<p>Разгледай го спокойно. После ми кажи кое от изброеното ти тежи най-много — и говорим само по него.</p>
<p style="margin:22px 0"><a href="${SITE}/booking" style="display:inline-block;background:#1d2320;color:#fff;padding:12px 24px;border-radius:7px;text-decoration:none;font-weight:600">Запази 20 минути →</a></p>
<p>Отговори директно на този имейл, ако имаш въпрос — чета всичко лично.</p>`),
        text: `${hiT}

Радвам се, че остави телефона си на нашата реклама за AI автоматизации. Вместо да ти го обяснявам с думи, ето ти го наживо — отваря се от браузъра, нищо не се инсталира и нищо не се попълва:

Отвори демото: ${demo}

Линкът е твой — по него виждам, че си влязъл, и следващия път говорим по конкретното, вместо да започваме отначало.

Вътре работят 16 агента едновременно. Ето какво поемат:

• Запитвания и клиенти. Хващат лийда от рекламата, сайта или обаждането, оценяват го от 0 до 100, отговарят за секунди по имейл, чат и Messenger, и връщат замълчалите — за да не изтича нищо между пръстите.

• Документи и пари. Сглобяват офертата и я пращат като PDF, издават фактурата, следят кое е платено и кое още не е.

• Реклами и съдържание. Пишат постовете, генерират рекламните видеа от текст, публикуват в пет мрежи наведнъж, пълнят календара за седмицата и местят бюджета към кампанията, която печели.

• Ти, с глас. Казваш какво е станало по телефона — системата го записва, насрочва го и го изпълнява. Без да сядаш пред екран.

Строим и неща извън демото: следене на камери и обекти, връзка с машини, автономни задачи, които вървят сами през нощта.

Разгледай го спокойно. После ми кажи кое от изброеното ти тежи най-много — и говорим само по него.
